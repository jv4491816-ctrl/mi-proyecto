...
function showPage(pageId) {
    ...
    if (pageId === 'afinacion-tips') {
        ...
        if (adminPanelState.isInitialized) {
            cleanupAdminPanel();
        }
        // Inicializar nuevo panel
        initAdminPanel();
    } else if (pageId === 'foro') {
...
// Configuración global para estado de usuarios
const USER_STATUS_CONFIG = {
    ONLINE_THRESHOLD: 1 * 60 * 1000, // 1 minuto para "En línea"
    INACTIVE_THRESHOLD: 5 * 60 * 1000, // 5 minutos para "Inactivo"
    AWAY_THRESHOLD: 30 * 60 * 1000, // 30 minutos para "Ausente"
    OFFLINE_THRESHOLD: 7 * 24 * 60 * 60 * 1000, // 7 días para "Desconectado"
    CHECK_INTERVAL: 10000 // Verificar cada 10 segundos
};

// Variables para gestión de estado del panel
let adminPanelState = {
    isInitialized: false,
    lastUpdateTime: null,
    chartInstances: [],
...
async function initAdminPanel() {
            ...
            <tr>
                <td colspan="7" style="text-align: center; padding: 40px;">
                    <div class="loading-container">
                        ...
                        <p style="margin-top: 15px; color: var(--text-white);">Cargando panel de administración...</p>
                    </div>
                </td>
            </tr>
        `;
    ...
    adminPanelState.isInitialized = true;
    adminPanelState.lastUpdateTime = new Date();
}

// Función para crear elementos faltantes
function createMissingAdminElements() {
    ...
    const neededElements = {
        'user-search': '<div class="search-box"><input type="text" id="user-search" placeholder="Buscar por email o nombre..."><i class="fas fa-search"></i></div>',
        'search-results': '<div id="search-results" style="font-size: 0.9rem; color: rgba(255,255,255,0.7); margin-top: 5px;"></div>',
        'last-update-time': '<span id="last-update-time" style="font-size: 0.8rem; color: rgba(255,255,255,0.6);">Última actualización: --:--:--</span>'
    };
    
    Object.keys(neededElements).forEach(id => {
        if (!document.getElementById(id)) {
            // Insertar después del contenedor de controles
            const adminControls = document.querySelector('.admin-controls');
            if (adminControls && id === 'search-results') {
                adminControls.insertAdjacentHTML('afterend', neededElements[id]);
            }
        }
    });
    
    // Asegurar que las celdas de gráficos existan
    const statsGrid = document.querySelector('.stats-grid');
    if (statsGrid) {
        const chartIds = ['userStatusChart', 'userRolesChart', 'registrationTrendChart'];
        chartIds.forEach(id => {
            if (!document.getElementById(id)) {
...
async function loadAllUsersEnhanced() {
    console.log("📊 CARGANDO USUARIOS CON DATOS REALES...");
    ...
    usersList.innerHTML = `
        <tr>
            <td colspan="7" style="text-align: center; padding: 40px;">
                <div class="loading-container">
                    <i class="fas fa-spinner fa-spin fa-2x" style="color: var(--primary-blue);"></i>
                    <p style="margin-top: 15px; color: var(--text-white);">Cargando datos de usuarios...</p>
                </div>
            </td>
        </tr>
    `;
    ...
    try {
        ...
        if (firebase.isAvailable && firebase.db) {
            ...
            usersSnapshot.forEach(doc => {
                ...
                const processedUser = processUserData(userData, doc.id);
                users.push(processedUser);
            });
            
            console.log(`✅ ${users.length} usuarios obtenidos desde Firestore`);
            
        } else {
            console.log("📋 Usando datos de demostración (modo offline)");
            users = getDemoUsersData();
        }
        
        // Actualizar estado global
        adminPanelState.usersData = users;
        
        // Procesar estadísticas
        processUserStatistics(users);
        
        // Actualizar UI
        updateUsersTableEnhanced(users);
        updateAllStats();
        
        // Guardar en caché
        userCache.set('allUsers', users);
        
        console.log("📈 Datos de usuarios procesados correctamente");
        ...
        usersList.innerHTML = `
            <tr>
                <td colspan="7" style="text-align: center; padding: 40px; color: var(--accent-red);">
                    ...
                    <p style="margin-top: 15px;">Error al cargar usuarios: ${error.message}</p>
                    <button onclick="loadAllUsersEnhanced()" class="admin-btn" style="margin-top: 15px;">
...
// ========== PROCESAR DATOS DE USUARIO CON ESTADOS CORRECTOS ==========
// ========== PROCESAR DATOS DE USUARIO CON ESTADOS CORRECTOS ==========
// ========== PROCESAR DATOS DE USUARIO CON ESTADOS CORRECTOS ==========
function processUserData(userData, userId) {
    const now = new Date();
    
    // Procesar fechas
    const createdAt = userData.createdAt?.toDate ? userData.createdAt.toDate() : new Date(userData.createdAt || now);
    const lastLogin = userData.lastLogin?.toDate ? userData.lastLogin.toDate() : new Date(userData.lastLogin || null);
    
    // Obtener usuario actual
    const currentUserData = JSON.parse(localStorage.getItem('guitarraFacilUser'));
    const isCurrentUser = currentUserData && userData.email === currentUserData.email;
    
    // SI ES EL USUARIO ACTUAL → SIEMPRE EN LÍNEA (MIENTRAS USA LA PÁGINA)
    if (isCurrentUser) {
        return {
            id: userId || `user-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
            email: userData.email || 'sin-email',
            displayName: userData.displayName || userData.name || userData.email?.split('@')[0] || 'Usuario',
            role: userData.role || 'student',
            photoURL: userData.photoURL || null,
            provider: userData.provider || 'email',
            progress: userData.progress || { level: 1, percentage: 0, lessons: 0 },
            createdAt: createdAt,
            lastLogin: now, // TIEMPO ACTUAL
            isActive: true,
            activityStatus: 'online',
            lastActivityText: 'En línea ahora',
            isNewToday: isToday(createdAt)
        };
    }
    ...
    if (lastLogin && !isNaN(lastLogin.getTime())) {
        ...
        lastActivityText = calculateLastActivityText(lastLogin);
        
        // Determinar estado según umbrales
        if (timeSinceLastLogin <= USER_STATUS_CONFIG.ONLINE_THRESHOLD) {
            activityStatus = 'online';
            isActive = true;
        } else if (timeSinceLastLogin <= USER_STATUS_CONFIG.INACTIVE_THRESHOLD) {
            activityStatus = 'active';
            isActive = true;
        } else if (timeSinceLastLogin <= USER_STATUS_CONFIG.AWAY_THRESHOLD) {
            activityStatus = 'inactive';
            isActive = false;
        } else if (timeSinceLastLogin <= USER_STATUS_CONFIG.OFFLINE_THRESHOLD) {
            activityStatus = 'away';
            isActive = false;
        } else {
...
// ========== CALCULAR TEXTO DE ÚLTIMA ACTIVIDAD PRECISO ==========
function calculateLastActivityText(date) {
    ...
    // Mostrar el tiempo más apropiado
    if (diffSeconds < 60) {
...
// ========== DATOS DE DEMOSTRACIÓN CON ESTADOS REALES ==========
// ========== DATOS DE DEMOSTRACIÓN CON ESTADOS REALES ==========
function getDemoUsersData() {
    const now = new Date();
    const currentUserData = JSON.parse(localStorage.getItem('guitarraFacilUser'));
    ...
    return [
        {
            ...
            lastLogin: new Date(), // AHORA para usuario actual
            isActive: true,
            activityStatus: currentUserEmail === 'jv4491816@gmail.com' ? 'online' : 'active',
            lastActivityText: currentUserEmail === 'jv4491816@gmail.com' ? 'En línea ahora' : 'Hace un momento',
            isNewToday: false
        },
        {
            id: '2',
            ...
            lastLogin: new Date(), // AHORA para usuario actual
            isActive: true,
            activityStatus: currentUserEmail === 'fraylingay@gmail.com' ? 'online' : 'active',
            lastActivityText: currentUserEmail === 'fraylingay@gmail.com' ? 'En línea ahora' : 'Hace 3 minutos',
            isNewToday: false
        },
        {
            id: '3',
            ...
            lastLogin: new Date(now.getTime() - 6 * 60 * 1000), // 6 minutos (INACTIVO)
            isActive: false,
            activityStatus: 'inactive',
            lastActivityText: 'Hace 6 minutos',
            isNewToday: false
        },
        {
            id: '4',
            ...
            lastLogin: new Date(now.getTime() - 1 * 60 * 60 * 1000), // 1 hora (AUSENTE)
            isActive: false,
            activityStatus: 'away',
            lastActivityText: 'Hace 1 hora',
            isNewToday: false
        },
        {
            id: '5',
            email: 'desconectado@demo.com',
            displayName: 'Usuario Desconectado',
            role: 'student',
            photoURL: '',
            provider: 'email',
            progress: { level: 1, percentage: 5, lessons: 1 },
            createdAt: new Date(now.getTime() - 10 * 24 * 60 * 60 * 1000),
            lastLogin: new Date(now.getTime() - 8 * 24 * 60 * 60 * 1000), // 8 días (DESCONECTADO)
            isActive: false,
            activityStatus: 'offline',
            lastActivityText: 'Hace 8 días',
            isNewToday: false
        },
        {
            id: '6',
            ...
            isActive: true,
            activityStatus: 'online',
            lastActivityText: 'Hace un momento',
            isNewToday: true
        }
    ];
}

// ========== ACTUALIZAR TABLA DE USUARIOS MEJORADA ==========
function updateUsersTableEnhanced(users) {
    const usersList = document.getElementById('users-list');
    if (!usersList) {
        console.error("❌ No se encontró el elemento users-list");
        return;
    ...
    if (!users || users.length === 0) {
        usersList.innerHTML = `
            <tr>
                <td colspan="7" style="text-align: center; padding: 40px; color: var(--text-white);">
                    <i class="fas fa-users-slash fa-2x"></i>
                    <p style="margin-top: 15px;">No hay usuarios registrados</p>
                </td>
            </tr>
        `;
        return;
    ...
    users.forEach(user => {
        const row = createUserTableRow(user, currentUser);
...
function createUserTableRow(user, currentUser) {
    const row = document.createElement('tr');
    row.dataset.userId = user.id;
    row.dataset.userEmail = user.email;
    
    // Avatar y nombre
    const avatarCell = createAvatarCell(user, currentUser);
    
    // Información básica
    const email = user.email || 'sin-email';
    const registerDate = user.createdAt.toLocaleDateString('es-ES');
    
    // Estado de actividad con iconos
    const statusCell = createStatusCell(user);
    
    // Rol con badge mejorado
    const roleCell = createRoleCell(user);
    
    // Último acceso con tooltip
    const lastAccessCell = createLastAccessCell(user);
    
    // Acciones
    const actionsCell = createActionsCell(user, currentUser);
    
    row.innerHTML = `
        <td>${avatarCell}</td>
        <td class="user-email">${email}</td>
        <td class="register-date">${registerDate}</td>
        <td>${lastAccessCell}</td>
        <td>${roleCell}</td>
        <td>${statusCell}</td>
        <td>${actionsCell}</td>
    `;
...
function createAvatarCell(user, currentUser) {
    ...
    if (user.photoURL) {
        avatarContent = `<img src="${user.photoURL}" alt="${displayName}" class="user-avatar-img">`;
    } else {
        avatarContent = `<div class="avatar-initials" style="background-color: ${avatarColor}">${initials}</div>`;
    }
    ...
    return `
        <div class="user-info-cell">
            <div class="user-avatar">
                ${avatarContent}
                ${isCurrentUser ? '<span class="current-user-indicator" title="Tú"><i class="fas fa-user-circle"></i></span>' : ''}
            </div>
            <div class="user-details">
                <div class="user-name">
                    ${displayName}
                    ${user.role === 'admin' ? '<i class="fas fa-crown admin-crown" title="Administrador"></i>' : ''}
                </div>
            </div>
        </div>
    `;
...
function createStatusCell(user) {
    ...
    let statusIcon = '';
    let statusText = '';
    let tooltipText = user.lastActivityText;
    ...
    switch(user.activityStatus) {
        case 'online':
            ...
            statusText = 'En línea';
            tooltipText = 'Conectado ahora mismo';
            break;
        case 'active':
            statusClass = 'status-active';
            statusIcon = '<i class="fas fa-circle"></i>';
            statusText = 'Activo';
            tooltipText = 'Activo recientemente';
            break;
        case 'inactive':
            statusClass = 'status-inactive';
            statusIcon = '<i class="fas fa-circle"></i>';
            statusText = 'Inactivo';
            tooltipText = 'Inactivo por más de 5 minutos';
            break;
        case 'away':
            statusClass = 'status-away';
            statusIcon = '<i class="fas fa-circle"></i>';
            statusText = 'Ausente';
            tooltipText = 'Ausente por más de 30 minutos';
            break;
        case 'offline':
            statusClass = 'status-offline';
            statusIcon = '<i class="fas fa-circle"></i>';
            statusText = 'Desconectado';
            tooltipText = 'Desconectado por más de 7 días';
            break;
        default:
            statusClass = 'status-unknown';
            statusIcon = '<i class="fas fa-circle"></i>';
            statusText = 'Desconocido';
            tooltipText = 'Estado no disponible';
    }
    
    return `
        <div class="status-cell ${statusClass}" title="${tooltipText}">
...
function createRoleCell(user) {
    ...
    const roleIcon = isAdmin ? '<i class="fas fa-shield-alt"></i>' : '<i class="fas fa-user-graduate"></i>';
...
// ========== CREAR CELDA DE ÚLTIMO ACCESO ==========
function createLastAccessCell(user) {
    return `
        <div class="last-access-cell" title="Se conectó: ${user.lastActivityText}">
            <i class="far fa-clock"></i>
            <span>${user.lastActivityText}</span>
        </div>
    `;
}

// ========== CREAR CELDA DE ACCIONES ==========
function createActionsCell(user, currentUser) {
    const isCurrentUser = user.email === currentUser?.email;
    const isAdmin = user.role === 'admin';
    
    return `
        <div class="user-actions">
            <button class="action-icon-btn view" onclick="viewUserDetails('${user.id}')" title="Ver detalles">
                <i class="fas fa-eye"></i>
            </button>
            <button class="action-icon-btn edit" onclick="editUser('${user.id}')" title="Editar usuario">
                <i class="fas fa-edit"></i>
            </button>
            <button class="action-icon-btn role" onclick="toggleUserRole('${user.id}', '${user.email}')" 
                    title="${isAdmin ? 'Quitar admin' : 'Hacer admin'}">
                <i class="fas ${isAdmin ? 'fa-user-minus' : 'fa-user-shield'}"></i>
            </button>
            ${!isCurrentUser ? `
                <button class="action-icon-btn delete" onclick="deleteUser('${user.id}', '${user.email}')" title="Eliminar usuario">
                    <i class="fas fa-trash"></i>
                </button>
            ` : `
                <button class="action-icon-btn disabled" title="No puedes eliminarte a ti mismo" disabled>
                    <i class="fas fa-trash"></i>
                </button>
            `}
        </div>
...
function processUserStatistics(users) {
    if (!users || users.length === 0) {
    ...
    adminPanelState.stats = {
        ...
        onlineUsers: users.filter(u => u.activityStatus === 'online').length,
        activeUsers: users.filter(u => u.activityStatus === 'active').length,
        inactiveUsers: users.filter(u => u.activityStatus === 'inactive').length,
        awayUsers: users.filter(u => u.activityStatus === 'away').length,
        offlineUsers: users.filter(u => u.activityStatus === 'offline').length,
...
function updateAllStats() {
    if (!adminPanelState.stats) return;
    
    const stats = adminPanelState.stats;
    
    // Actualizar todos los contadores
    updateCounter('total-users', stats.totalUsers);
    updateCounter('active-users', stats.activeUsers + stats.onlineUsers);
    updateCounter('new-today', stats.newToday);
    updateCounter('student-count', stats.students);
    updateCounter('admin-count', stats.admins);
    updateCounter('online-users', stats.onlineUsers);
    updateCounter('active-count', stats.activeUsers);
    updateCounter('inactive-count', stats.inactiveUsers);
    updateCounter('away-count', stats.awayUsers);
    updateCounter('offline-count', stats.offlineUsers);
    updateCounter('recent-activity', stats.newToday);
...
function createUsersActivityChart() {
    const ctx = document.getElementById('userStatusChart');
    if (!ctx) {
        console.error("❌ No se encontró el canvas para userStatusChart");
        return;
    }
    ...
    const activityColors = [
        'rgba(76, 175, 80, 0.8)',   // Verde: En línea
        'rgba(33, 150, 243, 0.8)',  // Azul: Activos
        'rgba(255, 193, 7, 0.8)',   // Amarillo: Inactivos
        'rgba(255, 87, 34, 0.8)',   // Naranja: Ausentes
        'rgba(158, 158, 158, 0.8)'  // Gris: Desconectados
    ];
    
    // Destruir instancia anterior si existe
    if (adminPanelState.chartInstances[0]) {
        adminPanelState.chartInstances[0].destroy();
    }
    ...
    const chart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                borderColor: activityColors.map(color => color.replace('0.8', '1')),
                borderWidth: 2,
                hoverOffset: 15
            }]
        },
                ...
                tooltip: {
                    ...
                    callbacks: {
                        label: function(context) {
                            ...
                            const percentage = Math.round((value / stats.totalUsers) * 100);
    ...
    adminPanelState.chartInstances[0] = chart;
}

// ========== CREAR GRÁFICO DE ROLES DE USUARIOS ==========
function createUsersRoleChart() {
    const ctx = document.getElementById('userRolesChart');
    if (!ctx) {
        console.error("❌ No se encontró el canvas para userRolesChart");
        return;
    }
    ...
    // Destruir instancia anterior si existe
    if (adminPanelState.chartInstances[1]) {
        adminPanelState.chartInstances[1].destroy();
    }
    ...
    const chart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: [
                    'rgba(255, 193, 7, 0.8)',   // Amarillo para admins
                    'rgba(33, 150, 243, 0.8)'   // Azul para estudiantes
                ],
                borderColor: [
                    'rgba(255, 193, 7, 1)',
                    'rgba(33, 150, 243, 1)'
    ...
    adminPanelState.chartInstances[1] = chart;
}

// ========== CREAR GRÁFICO DE TENDENCIA DE REGISTROS ==========
function createRegistrationTrendChart() {
    const ctx = document.getElementById('registrationTrendChart');
    if (!ctx) {
        console.error("❌ No se encontró el canvas para registrationTrendChart");
        return;
    }
    
    const users = adminPanelState.usersData;
    
    // Calcular registros por día (últimos 7 días)
    const last7Days = [];
    ...
    for (let i = 6; i >= 0; i--) {
        ...
        users.forEach(user => {
            ...
            if (userDate.toDateString() === date.toDateString()) {
                registrationsByDay[6 - i]++;
            }
        });
    }
    
    // Destruir instancia anterior si existe
    if (adminPanelState.chartInstances[2]) {
        adminPanelState.chartInstances[2].destroy();
    }
    ...
    adminPanelState.chartInstances[2] = chart;
}

// ========== DESTRUIR GRÁFICOS ==========
function destroyCharts() {
    adminPanelState.chartInstances.forEach((chart, index) => {
        if (chart && typeof chart.destroy === 'function') {
            chart.destroy();
            adminPanelState.chartInstances[index] = null;
        }
    });
    adminPanelState.chartInstances = [];
}
...
function setupSearchFunctionality() {
    ...
    searchInput.addEventListener('input', function() {
        ...
        searchTimeout = setTimeout(() => {
            ...
            filterUsersEnhanced(searchTerm);
        }, 300);
    });
}

// ========== FILTRAR USUARIOS MEJORADO ==========
function filterUsersEnhanced(searchTerm) {
    if (!searchTerm) {
        updateUsersTableEnhanced(adminPanelState.usersData);
        return;
    }
    
    const filteredUsers = adminPanelState.usersData.filter(user => 
        (user.email && user.email.toLowerCase().includes(searchTerm)) ||
        (user.displayName && user.displayName.toLowerCase().includes(searchTerm)) ||
        (user.role && user.role.toLowerCase().includes(searchTerm)) ||
        (user.activityStatus && user.activityStatus.toLowerCase().includes(searchTerm))
    );
    
    updateUsersTableEnhanced(filteredUsers);
    
    // Mostrar resultado de búsqueda
    const searchResults = document.getElementById('search-results');
    if (searchResults) {
        searchResults.textContent = filteredUsers.length === 0 ? 
            `No se encontraron usuarios para "${searchTerm}"` :
            `${filteredUsers.length} usuario${filteredUsers.length !== 1 ? 's' : ''} encontrado${filteredUsers.length !== 1 ? 's' : ''}`;
    }
}

// ========== ACTUALIZAR UI DEL ADMIN ==========
function updateAdminUI(userData) {
    try {
        const updates = [
            { id: 'admin-username', value: userData.firstName || userData.name || 'Administrador' },
            { id: 'admin-email', value: userData.email || '' }
        ];
        ...
        if (timestampElement) {
            timestampElement.textContent = `Última actualización: ${new Date().toLocaleTimeString('es-ES', {
                hour: '2-digit',
                minute: '2-digit',
                second: '2-digit'
            })}`;
...
async function toggleUserRole(userId, userEmail) {
    ...
    try {
        const user = adminPanelState.usersData.find(u => u.id === userId);
        if (!user) return;
        ...
        processUserStatistics(adminPanelState.usersData);
        updateUsersTableEnhanced(adminPanelState.usersData);
        updateAllStats();
        
        // Actualizar gráficos
        if (adminPanelState.chartInstances[1]) {
            adminPanelState.chartInstances[1].data.datasets[0].data = [
                adminPanelState.stats.admins,
                adminPanelState.stats.students
            ];
            adminPanelState.chartInstances[1].update();
        }
        
        showNotification(`✅ ${userEmail} ahora es ${newRole === 'admin' ? 'administrador' : 'estudiante'}`, 'success');
...
async function deleteUser(userId, userEmail) {
    if (!confirm(`¿Eliminar permanentemente a ${userEmail}?\nEsta acción no se puede deshacer.`)) {
        return;
    }
    ...
    try {
        ...
        // Eliminar localmente
        adminPanelState.usersData = adminPanelState.usersData.filter(u => u.id !== userId);
        ...
        // Actualizar gráficos
        adminPanelState.chartInstances.forEach(chart => {
            if (chart) chart.update();
        });
        
        showNotification(`✅ Usuario ${userEmail} eliminado`, 'success');
...
function viewUserDetails(userId) {
    ...
    const details = `
        <div style="text-align: left; max-width: 400px;">
            <h3 style="color: var(--primary-blue); margin-bottom: 15px;">${user.displayName}</h3>
            <div style="background: rgba(255,255,255,0.05); padding: 15px; border-radius: 8px;">
                <p><strong>📧 Email:</strong> ${user.email}</p>
                <p><strong>👑 Rol:</strong> ${user.role === 'admin' ? 'Administrador' : 'Estudiante'}</p>
                <p><strong>📱 Estado:</strong> ${getStatusText(user.activityStatus)}</p>
                <p><strong>⏰ Última conexión:</strong> ${user.lastActivityText}</p>
                <p><strong>📅 Registrado:</strong> ${user.createdAt.toLocaleDateString('es-ES')}</p>
                <p><strong>📊 Progreso:</strong> Nivel ${user.progress.level} (${user.progress.percentage}%)</p>
                <p><strong>📚 Lecciones completadas:</strong> ${user.progress.lessons}</p>
            </div>
        </div>
    `;
...
function cleanupAdminPanel() {
    if (adminPanelState.realTimeInterval) {
        clearInterval(adminPanelState.realTimeInterval);
        adminPanelState.realTimeInterval = null;
    }
    
    destroyCharts();
    
    adminPanelState.isInitialized = false;
    adminPanelState.lastUpdateTime = null;
    
    console.log("🧹 Panel de administración limpiado");
}

// ========== FUNCIONES PARA HERRAMIENTAS RÁPIDAS ==========
window.sendAnnouncement = function() {
    showNotification('Función de envío de anuncio en desarrollo', 'info');
};

window.exportUsers = function() {
    showNotification('Función de exportación de datos en desarrollo', 'info');
};

window.generateReport = function() {
    showNotification('Función de generación de reporte en desarrollo', 'info');
};

window.backupDatabase = function() {
    showNotification('Función de respaldo de base de datos en desarrollo', 'info');
};
...
async function refreshUserData() {
    if (!adminPanelState.isInitialized) return;
...
function simulateStatusChanges() {
    ...
    adminPanelState.usersData.forEach(user => {
        // Simular cambios aleatorios de estado para demostración
        if (Math.random() > 0.8) {
            // Cambiar el tiempo de última conexión
            const randomChange = Math.floor(Math.random() * 10) - 5; // -5 a +5 minutos
            const newLastLogin = new Date(user.lastLogin.getTime() + (randomChange * 60 * 1000));
            
            // Asegurarse de que no sea futuro
            if (newLastLogin <= now) {
                user.lastLogin = newLastLogin;
                user.lastActivityText = calculateLastActivityText(newLastLogin);
                
                // Recalcular estado
                const timeSinceLastLogin = now - newLastLogin;
...
function updateChartsWithNewData() {
    ...
    if (adminPanelState.chartInstances[0]) {
        adminPanelState.chartInstances[0].data.datasets[0].data = [
            stats.onlineUsers,
            stats.activeUsers,
            stats.inactiveUsers,
            stats.awayUsers,
        ...
        adminPanelState.chartInstances[0].update('none');
    }
    
    // Actualizar gráfico de roles
    if (adminPanelState.chartInstances[1]) {
        adminPanelState.chartInstances[1].data.datasets[0].data = [
            stats.admins,
            stats.students
        ];
        adminPanelState.chartInstances[1].update('none');
    }
    
    // Actualizar gráfico de tendencia (solo si hay cambios)
    if (adminPanelState.chartInstances[2]) {
        // Simular nuevos registros para la demostración
        if (Math.random() > 0.7) {
            const todayIndex = 6; // Último día del array
            adminPanelState.chartInstances[2].data.datasets[0].data[todayIndex]++;
            adminPanelState.chartInstances[2].update('none');
        }
    }
}

// ========== EXPORTAR FUNCIONES GLOBALES ==========
window.loadAllUsers = loadAllUsersEnhanced;
...
window.refreshUserStats = refreshUserData;

// ========== AFINADOR Y METRÓNOMO (MANTENIDOS IGUAL) ==========
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Reinicializar panel admin cuando se navegue a él
    document.addEventListener('click', function(e) {
        const link = e.target.closest('[data-page="admin-panel-page"]');
        if (link) {
            setTimeout(() => {
